...
%     saveas(gcf,'Comparison of percentage errors for increasing extinction coefficient percentage changes for broadband system 1.png')
    ...
    for u = 1:length(concs1) 
        for r = 1:length(x)
            if x(1) == 680 && r == 1
                [val, idx] = min(abs(E_new((681-wl(1)):(681-wl(1))+tolerance,u) - E_prime_laser(r,u)));
                new_ext_wavs_init(r,u) = idx;
                new_ext_wavs_laser(r,u) = new_ext_wavs_init(r,u) + 680;
            else
...
%             saveas(gcf,'Comparison of percentage errors for increasing extinction coefficient percentage changes for 5-wavelength laser system.png')
    ...
    %         conc_assDPF_Etake15_SNR30_laser = pinv(laser_E_take15) * (-change_I_laser_SNR30 ./ (SD*DPF_assumed))';
            ...
            perc_accum_errors_DPFE_laserSNR50 = abs(perc_accum_errors_DPFE_laserSNR50);
    ...
    %%
        ...
        pert_ave_error_laser{pert_number} = ave_vector_error_laserSNR50(:,:);
        pert_high_bar_error_laser{pert_number} = high_bar_vector_error_laserSNR50;
        pert_low_bar_error_laser{pert_number} = low_bar_vector_error_laserSNR50;
...
for u = 1:4
    for j = 1:3
        for k = 1:pert_number
            total_pert_error_laserSNR50(k,j) = pert_ave_error_laser{1,k}(u,j);
            high_bar_vector_error_laserSNR50(k,j) = pert_high_bar_error_laser{1,k}(u,j);
            low_bar_vector_error_laserSNR50(k,j) = pert_low_bar_error_laser{1,k}(u,j);
        end
        val_to_ave_error = total_pert_error_laserSNR50(:,j);
        high_to_ave_error = high_bar_vector_error_laserSNR50(:,j);
        low_to_ave_error = low_bar_vector_error_laserSNR50(:,j);
        ave_pert_error_laserSNR50(u,j) = mean(val_to_ave_error(val_to_ave_error<Inf));
        high_bar_pert_error_laserSNR50(u,j) = mean(high_to_ave_error(high_to_ave_error<Inf));
        low_bar_pert_error_laserSNR50(u,j) = mean(low_to_ave_error(low_to_ave_error<Inf));
    end
end
...
FigH = figure('Position', get(0, 'Screensize'));
F = getframe(FigH);
imwrite(F.cdata, 'Average percentage errors for increasing E and DPF changes for laser SNR 50 random conc.png', 'png')
errorbar(percent_vector,ave_pert_error_laserSNR50(:,1),low_bar_pert_error_laserSNR50(:,1),high_bar_pert_error_laserSNR50(:,1),'r*-','MarkerSize',20,'LineWidth',1.8)
hold on
errorbar(percent_vector,ave_pert_error_laserSNR50(:,2),low_bar_pert_error_laserSNR50(:,2),high_bar_pert_error_laserSNR50(:,2),'bs-','MarkerSize',20,'LineWidth',1.5)
errorbar(percent_vector,ave_pert_error_laserSNR50(:,3),low_bar_pert_error_laserSNR50(:,3),high_bar_pert_error_laserSNR50(:,3),'go-','MarkerSize',20,'LineWidth',1)
grid on
ax = gca;
ax.FontSize = 20;
xlim([-5 20]);
...
title({'Average percentage errors for increasing extinction coefficient', 'and DPF percentage changes for laser with random conc changes, SNR = 50'});
saveas(gcf,'Average percentage errors for increasing E and DPF changes for laser SNR 50 random conc.png')
