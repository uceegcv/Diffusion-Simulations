...
%         x = 682:xstep:921;
        ...
        compiled_E_newdata = compiled_E_newdata.';
        compiled_E_newdata = compiled_E_newdata/((2.3^2)*1000000);
        compiled_E_wavs = 650:1000;

        %% Lorentzian

        % Approximate LED shape as a Lorentzian function using different bandwidths
        for i = 1:length(compiled_E_wavs)
            for j = 1:length(x)
                LED_spec(i,j) = (1/pi)*((w_LED(j)/2)/(((compiled_E_wavs(i)-x(j))^2)+(w_LED(j)/2)^2));
            end
        end
        ...
        for l = 1:length(ac_conc_change) %chromophores 
            for j = 1:length(x) % wavelengths
                for k = 1:length(compiled_E_wavs) % broadband
                    e_effective(k,l) = compiled_E_newdata(k,l)*LED_spec(k,j); % column = chromophore, row = wavelength
                end
        ...
        for g = 1:length(x)
            ave_DPF_LED(g) = ave_DPF_BB(x(g)-wl(1));
        end
...
%                 R2_iteration(:,l) = R2_LED;
    ...
    num_wavs = length(x);
    save(['ave_error_',num2str(num_wavs),'_3to200wavelengths_differentEs_origDPF_noiseless'],'ave_pert_error_LEDSNR50','final_std_val_LEDSNR50')
