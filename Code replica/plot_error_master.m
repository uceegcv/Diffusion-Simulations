...
% theFiles = dir('/Users/georginaleadley/Documents/GitHub/Diffusion-Simulations/Code replica/Errors 3-200 wav correct std/*.mat');
theFiles = dir('\\ifs.eng.cam.ac.uk\users\gcl33\Documents\GitHub\Diffusion-Simulations\Code replica\Errors 1-200band with conv correct std\*.mat');
...
FigH = figure('Position', get(0, 'Screensize'));
F = getframe(FigH);
imwrite(F.cdata, 'Comparing 1-200 band ten perc parameter error shaded correct std zeroed second dataset.png', 'png')
...
ylabel('Average percentage error in chromophore concentration values');
%title({'Comparing 5-wavelength systems with increasing bandwidth and fifteen percent error in DPF and E'});
saveas(gcf,'Comparing 1-200 band ten perc parameter error shaded correct std zeroed second dataset.png')
