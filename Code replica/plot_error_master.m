...
theFiles = dir('/Users/georginaleadley/Documents/GitHub/Diffusion-Simulations/Code replica/full errors 5-200 wavs/no SNR/*.mat');
...
for k = 1 : length(theFiles)
    ...
    data_to_analyse = newdata.ave_pert_error_LEDnoSNR;
    std_to_analyse = newdata.final_std_val_LEDnoSNR;
...
FigH = figure('Position', get(0, 'Screensize'));
F = getframe(FigH);
imwrite(F.cdata, 'Comparing 3-200wav systems zero parameter error no SNR shaded.png', 'png')
...
ylabel('Average percentage error in chromophore concentration values');
%title({'Comparing 5-wavelength systems with increasing bandwidth and fifteen percent error in DPF and E'});
saveas(gcf,'Comparing 3-200wav systems zero parameter error no SNR shaded.png')
